...
b = json.load(open('test_inv1.json'))
		...
		for attribute in item['attributes']:
			try:
				...
				if attribute['defindex'] == 2000:
					for subattribute in attribute['defindex']:
						if subattribute['defindex'] == 2025:
							if subattribute['float_value'] == 2:
								sku += ';kt-3'
							elif subattribute['float_value'] == 1:
								sku += ';kt-2'
				try:
					if attribute['defindex'] > 2000 and attribute['defindex'] <= 2009 and attribute['is_output'] == True:
						for subattribute in attribute['attributes']:
							if subattribute['defindex'] == 2012:
								sku += ';td-{}'.format(subattribute['float_value'])
						sku += ';od-{};oq-6'.format(attribute['itemdef'], attribute['quality'])
				except:
					pass
			except:
				pass
